...
def sendFile(reqFile,connectionSocket): # Sends filesize, bytes with a tag
    ...
    with open(reqFile, "rb") as f:#opens file and reads and sends file byte data a chunk at a time(if the file is big enough)
        while True:
            data = b"<2><FILDATA>" +f.read(4084)#Read a set number of bytes from data
            #print(data)
            if data == b"<2><FILDATA>":#loops until no data is being sent
                break
 
            connectionSocket.sendall(data)
            hValidation.update(data[12:])#hash is updated with new data
...
def recvFile(connectionSocket):#method ran when client is uploading a file to the server
    ...
    with open(fName, "wb") as f: # Open this file in write-byte mode
        while True:
            ...
            f.write(data[12:]) # write chunk of data to file
            hValidation.update(data[12:]) # update the hash with this chunk of data
...
def main():#Main method ran on server start
    ...
    passPath = './Database/passwords.txt'
    isPassPath = os.path.isdir(path)
