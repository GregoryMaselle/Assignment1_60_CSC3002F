...
def downloadFile(fName, clientSocket):
  ...
  while not done:
    ...
    if byteData == b"<END>":
      ...
      file.write(byteData[12:]) # appends the received data to the file.
      clientHash.update(byteData[12:]) # updates the hash code to include the newly received data.
...
def sendFile(clientSocket):
  ...
  file = open(fName, "rb")
  while True:
    byteData = b"<2><FILDATA>" +file.read(4084) # reads 4096 bytes of data from the file at a time to be sent.
    if byteData == b"<2><FILDATA>":
      # once we have reached the end of the file, the file is closed and the loop is exited.
      file.close()
      break
    clientSocket.sendall(byteData)
    clientHash.update(byteData[12:]) #iteratively updates the hash code as the data is read from the file.
    msg = clientSocket.recv(1024).decode()
