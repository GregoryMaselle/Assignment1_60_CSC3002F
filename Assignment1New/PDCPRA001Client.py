...
def uploadFile(cSock):#Method called when client wants to upload to server
	...
	with open(fName, "rb") as f:#Sends file data to server piece by piece
		while True:
			...
			hvalidation.update(fileData)#Hash is updated with new data
	...
	with open(name, "wb") as f:
		while True:#Runs until the end of the file
			...
			if fileData == b"<END>":#end condition
				break
			f.write(fileData)#every piece of data got is written to the file on the client side
			hValidation.update(fileData)#Hash is updated with current data
