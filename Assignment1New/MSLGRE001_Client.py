...
def receiveData(fileName, clientSocket):
  ...
  with open(fileName, "wb") as f:
    while True: 
      ...
      f.write(data) # write the data to the intended file.
      hashToCheck.update(data) # update the hash for every chunk of data.
...
def clientUploadControl(clientSocket): # A method used to manage passwords and names for uploading files to server.
  ...
  with open(fileName, "rb") as f: # open the target file on client machine.
      while True: 
          data = f.read(4096) # read out a chunk of data from the desired file.
 
          if not data: # when the end of file is reached, stop sending chunks.
              break
 
          clientSocket.sendall(data)# Send the chunk of data to the server.
          hashToCheck.update(data)# Update the hash with new data.
