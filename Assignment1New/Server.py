...
def multiThreadClient(connection, address):

    while True:
        msg = "<0><CONNSCS>Client Connected; IP: "+address[0]+"\n"+"Type X to access a file, type Y to upload a file or, type Q to quit."
        fileList = os.listdir("./Database")
        connection.send(msg.encode(MSGFORMAT))
        ans = connection.recv(1024).decode()[12:]
        print(ans)
        if ans == "X":#If the client chooses to download a file
            ...
            for filename in fileList:#loop to display all of the files on the server, the passwords.txt file is hidden
                if filename == "passwords.txt":
                    continue
                fileDisp += filename + "\n"
            connection.send(fileDisp.encode(MSGFORMAT))#
            reqFile = connection.recv(1024).decode()[12:]
            findAndSend(reqFile,connection)#the file the client would like to download is used as a parameter
...
def main():#Main method ran on server start
    ...
    while True:
        ...
        start_new_thread(multiThreadClient, (connectionSocket, addr, ))
        threadCount+=1
