...
def sendFile(reqFile,connectionSocket): # Sends filesize, bytes with a tagasd
    ...
    #     connectionSocket.sendall(b"<2><SFILSEN>"+data+b"<END>")
        ...
        file.close()
        statusMessage = connectionSocket.recv(1024).decode()
        print(statusMessage+"STATUS MESSAGE")
        if (statusMessage[3:13]!="<HSHFAIL>"):
            successful = True
            connectionSocket.send("<0><HSHSCCS>Hash was successful.".encode())
            os.chdir("../")
            print("Successful")
